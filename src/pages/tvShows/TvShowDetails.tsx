...
const getTvShowDetails = async (id: string | number | undefined) => {
...
const TvShowDetails = () => {
  let { id } = useParams();
  const { data, status } = useQuery(["movieDetails", id], () => getTvShowDetails(id), { refetchOnWindowFocus: false });
    ...
    <section>
        ...
        >
          ...
          <div className="container mx-auto flex min-h-screen flex-col items-center gap-8 px-4 py-16 lg:flex-row lg:px-0 lg:py-0">
            ...
            <section className="flex-2 z-10">
              {data.poster_path ? (
                <img src={imgUrl + data.poster_path} className="mx-auto w-64 rounded md:w-80" alt={data.name} />
              ) : (
            ...
            <section className="z-10 flex-1">
              ...
              <h2 className="text-2xl font-bold lg:text-3xl">
                {data.name} <span className="font-normal text-gray-300">({shortFormattedDate(data.first_air_date)})</span>
              </h2>
      ...
      {status === "success" && (
        <>
          ...
          {/* Similar movies */}
          <Similar type="tv" showPath="tv-show-details" />
        </>
      )}
    </section>
  );
